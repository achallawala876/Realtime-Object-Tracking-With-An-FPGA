...
while True:
    dev.ActivateTriggerIn(0x40, 0); #Reset FIFOs and counter
    dev.ReadFromBlockPipeOut(0xa0, 32, buf);  # Read data from BT PipeOut   
    dev.SetWireInValue(0x09, 0) # reset motor pulse counter
    dev.UpdateWireIns() 
    ...
    frame_count += 1
    elapsed_time = time.time() - start_time+0.01
    ...
    cv2.putText(frame, "FPS : " + str(fps), (10,50), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (50,170,50), 1);    
    ...
    if (int(320-p1[0])>20):
        dev.SetWireInValue(0x10, int(10)) # set 10 pulses for this voltage
        ...
        dev.SetWireInValue(0x10, int(10)) # set 10 pulses for this voltage  
        dev.SetWireInValue(0x09, 1) # start motor controller
        # print("right")
        dev.UpdateWireIns()
